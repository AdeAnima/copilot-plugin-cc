...
function normalizeMaxInlineFiles(value) {
  ...
  if (!Number.isFinite(parsed) || parsed < 0) return DEFAULT_INLINE_DIFF_MAX_FILES;
  return Math.floor(parsed);
}

function normalizeMaxInlineDiffBytes(value) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed < 0) return DEFAULT_INLINE_DIFF_MAX_BYTES;
  return Math.floor(parsed);
}
...
function measureCombinedGitOutputBytes(cwd, argSets, maxBytes) {
  ...
  for (const args of argSets) {
    if (totalBytes > maxBytes) return totalBytes;
    totalBytes += measureGitOutputBytes(cwd, args, maxBytes - totalBytes);
  }
  return totalBytes;
}
...
function collectStagedContext(cwd, state, options = {}) {
  ...
  return {
    ...
    content: [
      formatSection("Staged Diff Stat", stagedStat),
      formatSection("Changed Files", state.staged.join("\n"))
    ].join("\n"),
    changedFiles: state.staged
  };
}
...
export function collectReviewContext(cwd, target, options = {}) {
  ...
  return {
    ...
    target,
    fileCount: details.changedFiles?.length ?? state.staged.length,
    diffBytes,
